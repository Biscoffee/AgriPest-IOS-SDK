...
#import "AGJSONRequestSerializer.h"
#import "AGQueryParamCollection.h"
#import "AGDefaultConfiguration.h"

NSString *const AGResponseObjectErrorKey = @"AGResponseObject";

static NSString * const kAGContentDispositionKey = @"Content-Disposition";

static NSDictionary * AG__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * AG__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = AG__headerFieldsForResponse(response);
    if(!headers[kAGContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kAGContentDispositionKey];
...
@interface AGApiClient ()

@property (nonatomic, strong, readwrite) id<AGConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation AGApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static AGApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[AGDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[AGDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithConfiguration:(id<AGConfiguration>)configuration {
    return [self initWithBaseURL:[NSURL URLWithString:configuration.host] configuration:configuration];
}

- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<AGConfiguration>)configuration {
    self = [super initWithBaseURL:url];
    if (self) {
        _configuration = configuration;
        _timeoutInterval = 60;
        _responseDeserializer = [[AGResponseDeserializer alloc] init];
        _sanitizer = [[AGSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        AGJSONRequestSerializer * swgjsonRequestSerializer = [AGJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kAGApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        AGDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[AGResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = AG__fileNameForResponse(response);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = AGPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, AGPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[AGQueryParamCollection class]]){
            AGQueryParamCollection * coll = (AGQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = AGPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = AGPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = AGPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)queries WithAuthSettings:(NSArray *)authSettings {
    ...
    id<AGConfiguration> config = self.configuration;
